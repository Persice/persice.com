...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        orm.Event.objects.filter(event_type='persice').update(eid=None)
        user_fb_ids = orm['django_facebook.FacebookCustomUser'].objects.all().values_list('facebook_id')
        events = orm.Event.objects.filter(event_type='facebook')
        for user_fb_id in user_fb_ids:
            for event in events:
                organizer = event.membership_set.filter(
                    is_organizer=True).first()

                fb_event = orm.FacebookEvent.objects.filter(facebook_id=event.eid).first()
                if not fb_event:
                    continue
                owner_info = json.loads(fb_event.owner)
                if organizer and owner_info.get('id', -1) == user_fb_id:
                    orm.Membership.objects.filter(
                        event=event, user=organizer.user).update(
                        is_organizer=True)
                elif organizer and organizer.user.facebook_id != user_fb_id:
                    orm.Membership.objects.filter(
                        event=event, user=organizer.user).update(
                        is_organizer=False)
    ...
    models = {
        ...
        u'events.membership': {
            ...
            'updated': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2016, 9, 26, 0, 0)'}),
            'user': ('django.db.models.fields.related.ForeignKey', [], {'to': u"orm['django_facebook.FacebookCustomUser']"})
        }
    }
