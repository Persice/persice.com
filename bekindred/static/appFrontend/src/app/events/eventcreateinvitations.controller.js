  ...
  function EventCreateInvitationsController($scope, USER_ID, EventsFactory, EventsAttendees, MembersFactory, $q, EventsConnections, $state, $timeout, $rootScope, $log, $filter, notify, lodash) {
    ...
    function markSelected(index, id) {
      if (!vm.connections[index].is_invited) {
        ...
        if (vm.connections[index].selected && $scope.singleevent.selectedPeople.indexOf(id) === -1) {
          $scope.singleevent.selectedPeople.push(id);
        }
        if (!vm.connections[index].selected && $scope.singleevent.selectedPeople.indexOf(id) !== -1) {
          $scope.singleevent.selectedPeople = lodash.without($scope.singleevent.selectedPeople, id);
