...
@Directive({
  selector: 'google-map-circle'
})
export class GoogleMapCircle implements OnInit, OnChanges, OnDestroy {
  ...
  private _updateCircleOptionsChanges(changes: {[propName: string]: SimpleChange}) {
    ...
    let optionKeys =
      Object.keys(changes).filter(k => GoogleMapCircle._mapOptions.indexOf(k) !== -1);
