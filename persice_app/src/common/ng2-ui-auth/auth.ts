...
export function NG2_UI_AUTH_PROVIDERS(config: ICustomConfig): Array<any> {
  return [{provide: Config, useFactory: () => { return new Config(config);}},
    ...
    {
      provide: Shared,
      useFactory: (storage, providedConfig) => { return new Shared(storage, providedConfig);},
      deps: [Storage, Config]
    },
    {
      provide: JwtHttp,
    ...
    {
      provide: Oauth,
      useFactory: (http, injector, shared, providedConfig) => { return new Oauth(http, injector, shared, providedConfig);},
      deps: [JwtHttp, Injector, Shared, Config]
    },
    ...
    {
      provide: Local,
      useFactory: (http, shared, providedConfig) => { return new Local(http, shared, providedConfig);},
      deps: [JwtHttp, Shared, Config]
    },
    {
      provide: Auth,
      useFactory: (shared, local, oauth) => { return new Auth(shared, local, oauth);},
      deps: [Shared, Local, Oauth]
    },
  ];
}
