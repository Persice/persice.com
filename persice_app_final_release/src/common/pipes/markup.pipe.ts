...
export class MarkupPipe implements PipeTransform {
  ...
  private eventInfoRegex: any = /\[eventStart\].+?\[eventEnd\]/g; // anything inside [eventStart][eventEnd] tags
  ...
  private applyMarkupLinks(value: string) {
    ...
    let match: any = regExp.exec(message);
  ...
  private formatEventInfoFromJson(message: string, eventPart: string[]) {
    ...
    // Format event info.
    result = message + result.replace(/\[eventStart\].+?\[eventEnd\]/g, EventWidget.asHtml(event));
